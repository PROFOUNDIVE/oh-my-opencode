# OpenMath Orchestrator Configuration
...
```jsonc
{
  "$schema": "https://raw.githubusercontent.com/code-yeongyu/oh-my-opencode/master/assets/oh-my-openmath.schema.json",
...
1. `.opencode/oh-my-openmath.jsonc` or `.opencode/oh-my-openmath.json` (project; prefers `.jsonc` when both exist)
2. User config (platform-specific; prefers `.jsonc` when both exist):

| Platform        | User Config Path                                                                                                            |
| --------------- | --------------------------------------------------------------------------------------------------------------------------- |
| **Windows**     | `~/.config/opencode/oh-my-openmath.jsonc` (preferred) or `~/.config/opencode/oh-my-openmath.json` (fallback); `%APPDATA%\opencode\oh-my-openmath.jsonc` / `%APPDATA%\opencode\oh-my-openmath.json` (fallback) |
| **macOS/Linux** | `~/.config/opencode/oh-my-openmath.jsonc` (preferred) or `~/.config/opencode/oh-my-openmath.json` (fallback)                |

> [!NOTE]
> **FAQ: Should I use `oh-my-openmath.jsonc` or `oh-my-opencode.jsonc`?**
>
> Use `oh-my-openmath.jsonc` for all new setups. `oh-my-opencode.json[c]` is legacy compatibility naming only and should appear only when migrating older installations.
>
> **FAQ: What about `oh-my-openagent`?**
> Keep `oh-my-openmath` and `oh-my-openagent` on distinct package/config names. The rename exists to avoid namespace collisions during migration.
...
The `oh-my-openmath` configuration file supports JSONC (JSON with Comments):
- Line comments: `// comment`
- Block comments: `/* comment */`
- Trailing commas: `{ "key": "value", }`

When both `oh-my-openmath.jsonc` and `oh-my-openmath.json` files exist, `.jsonc` takes priority.
...
Common Ollama models that work with OpenMath Orchestrator:
...
Or disable via `disabled_agents` in `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.json`:
...
OpenMath Orchestrator includes built-in skills that provide additional capabilities:
...
Disable built-in skills via `disabled_skills` in `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.json`:
...
| `compatibility`    | -       | Required oh-my-openmath version compatibility                                           |
...
**Switch providers** via `browser_automation_engine` in `oh-my-openmath.json`:
...
**Enable tmux integration** via `tmux` in `oh-my-openmath.json`:
...
2. When a background agent spawns, OpenMath Orchestrator creates a new tmux pane
...
```
1. User-configured model (in oh-my-openmath.json)
...
// oh-my-openmath.json (empty categories section)
{}
...
At runtime, OpenMath Orchestrator uses a 3-step resolution process to determine which model to use for each agent and category. This happens dynamically based on your configuration and available models.
...
1. **Step 1: User Override** — If you specify a model in `oh-my-openmath.json`, use exactly that
...
│   ┌─────────────────────────────────────────────────────────┐   │
│   │ User specified model in oh-my-openmath.json?            │   │
...
Override any agent or category model in `oh-my-openmath.json`:
...
Disable specific built-in hooks via `disabled_hooks` in `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.json`:
...
Disable specific built-in commands via `disabled_commands` in `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.json`:
...
Don't want them? Disable via `disabled_mcps` in `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.json`:
...
OpenMath Orchestrator adds refactoring tools (rename, code actions).
All OpenCode LSP configs and custom settings (from `opencode.jsonc` / `opencode.json`) are supported, plus additional OpenMath Orchestrator-specific settings.
For config discovery, `.jsonc` takes precedence over `.json` when both exist (applies to both `opencode.*` and `oh-my-openmath.*`).

Add LSP servers via the `lsp` option in `~/.config/opencode/oh-my-openmath.jsonc` / `~/.config/opencode/oh-my-openmath.json` or `.opencode/oh-my-openmath.jsonc` / `.opencode/oh-my-openmath.json`:
