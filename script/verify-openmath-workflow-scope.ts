...
const VOLATILE_GENERATED_BINARY_PATHS = new Set(["packages/darwin-arm64/bin/oh-my-openmath", "packages/darwin-x64-baseline/bin/oh-my-openmath", "packages/darwin-x64/bin/oh-my-openmath", "packages/linux-arm64-musl/bin/oh-my-openmath", "packages/linux-arm64/bin/oh-my-openmath", "packages/linux-x64-baseline/bin/oh-my-openmath", "packages/linux-x64-musl-baseline/bin/oh-my-openmath", "packages/linux-x64-musl/bin/oh-my-openmath", "packages/linux-x64/bin/oh-my-openmath", "packages/windows-x64-baseline/bin/oh-my-openmath.exe", "packages/windows-x64/bin/oh-my-openmath.exe"])
...
export function verifyOpenMathWorkflowScope(input: OpenMathWorkflowScopeFacts): readonly string[] {
  ...
  for (const path of lines(input.untrackedPaths)) {
    if (isVolatileUntrackedPath(path)) continue
...
function isVolatileUntrackedPath(path: string): boolean { return VOLATILE_GENERATED_BINARY_PATHS.has(path) || VOLATILE_OMO_PATHS.has(path) || VOLATILE_OMO_PREFIXES.some((prefix) => path.startsWith(prefix)) }
