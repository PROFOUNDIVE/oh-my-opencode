...
const PACKAGE_NAME = "oh-my-openmath"
...
console.log("=== Publishing oh-my-openmath (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-openmath-${platform}`
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/oh-my-openmath.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
