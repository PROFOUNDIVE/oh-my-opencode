 ...
 * @returns {string} Package name like "oh-my-openmath-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `oh-my-openmath-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "oh-my-openmath-darwin-arm64/bin/oh-my-openmath"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/oh-my-openmath${ext}`;
}
