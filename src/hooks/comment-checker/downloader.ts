...
/**
 * Get the cache directory for oh-my-openmath binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "oh-my-openmath", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[oh-my-openmath] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[oh-my-openmath] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[oh-my-openmath] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[oh-my-openmath] Comment checking disabled.`)
    return null
  }
}
