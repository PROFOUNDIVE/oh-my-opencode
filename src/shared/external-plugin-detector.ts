/**
 * Detects external plugins that may conflict with oh-my-openmath features.
...
/**
 * Known notification plugins that conflict with oh-my-openmath's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[oh-my-openmath] External notification plugin detected: ${pluginName}

Both oh-my-openmath and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   oh-my-openmath's session-notification has been auto-disabled.

   To use oh-my-openmath's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in oh-my-openmath.json`
}
